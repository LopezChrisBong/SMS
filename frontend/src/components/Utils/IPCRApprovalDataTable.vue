<template>
  <div>
    ...
    <ViewIPCRApprovalDialog :data="ipcr" :action="action" />
...
export default {
  components: {
    ViewIPCRApprovalDialog: () =>
      import("../../components/Dialogs/Views/ViewIPCRApprovalDialog.vue"),
  },
  data: () => ({
    search: "",
    headers: [
  ...
  mounted() {
    this.eventHub.$on("closeViewIPCRApprovalDialog", () => {
      this.initialize(this.tab);
    });
  },
  beforeDestroy() {
    this.eventHub.$off("closeViewIPCRApprovalDialog");
  },
  ...
  methods: {
    ...
    initialize(status) {
      this.loading = true;
      this.axiosCall("/ipcr/getIPCRByStatus/" + status, "GET").then((res) => {
        if (res) {
          let data = res.data;
