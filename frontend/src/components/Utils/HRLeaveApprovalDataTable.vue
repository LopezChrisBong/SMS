...
export default {
  components: {
    ViewHRLeaveApprovalDialog: () =>
      import("../../components/Dialogs/Views/ViewHRLeaveApprovalDialog.vue"),
  },
  data: () => ({
    search: "",
    headers: [
  ...
  mounted() {
    this.eventHub.$on("closeViewHRLeaveApprovalDialog", () => {
      this.initialize(this.tab);
    });
  },
  beforeDestroy() {
    this.eventHub.$off("closeViewHRLeaveApprovalDialog");
  },
  ...
  methods: {
    ...
    //       elementMap.set(element.id, elementMap.get(element) + 1);
    //     } else {
    //       elementMap.set(element.id, 1);
    //     }
    //   });
