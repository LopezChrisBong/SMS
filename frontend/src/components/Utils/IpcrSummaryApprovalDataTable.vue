...
export default {
  components: {
    ViewIPCRSummaryApprovalDialog: () =>
      import(
        "../../components/Dialogs/Views/ViewIPCRSummaryApprovalDialog.vue"
      ),
  },
  data: () => ({
    search: "",
  ...
  mounted() {
    this.eventHub.$on("closeIPCRSummaryDialog", () => {
      this.initialize(this.tab);
    });
  },
  beforeDestroy() {
    this.eventHub.$off("closeIPCRSummaryDialog");
  },
  ...
  methods: {
    ...
    initialize(status) {
      this.loading = true;
      this.axiosCall("/ipcr/getSummaryApprovalByStatus/" + status, "GET").then(
        (res) => {
          if (res) {
            let data = res.data;
