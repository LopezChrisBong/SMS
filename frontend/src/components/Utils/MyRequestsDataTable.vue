<template>
  <div>
    ...
    <v-card class="ma-5 dt-container" elevation="0" outlined>
      ...
      >
        <template v-slot:[`item.document_requested_id`]="{ item }">
          <div v-for="i in requestsList" :key="i.id">
            {{ i.id == item.document_requested_id ? i.type : "" }}
          </div>
        </template>
    ...
    <MyRequestsDialog :data="updateData" :action="action" :tab="tab" />
...
export default {
  components: {
    MyRequestsDialog: () =>
      import("../../components/Dialogs/Forms/MyRequestsDialog.vue"),
  },
  data: () => ({
    search: "",
    headers: [
  ...
  mounted() {
    this.eventHub.$on("closeMyRequestsDialog", () => {
      this.initialize(this.tab);
    });
  },
  beforeDestroy() {
    this.eventHub.$off("closeMyRequestsDialog");
  },
  ...
  methods: {
    ...
    initialize(id) {
      this.loading = true;
      if (id == 1) {
        this.axiosCall("/document-requests/getMyRequests", "GET").then(
          (res) => {
            if (res) {
