<template>
  <div>
    ...
    <v-card class="ma-5 dt-container" elevation="0" outlined>
      ...
      >
        ...
        <template v-slot:[`item.actions`]="{ item }">
          <v-btn
            x-small
            v-if="!getIsBeyondInclusiveDate(item.date_to)"
            outlined
            color="grey"
            @click="editItem(item)"
          >
    ...
    <MyOICDialog :data="updateData" :action="action" />
...
export default {
  components: {
    MyOICDialog: () => import("../../components/Dialogs/Forms/MyOICDialog.vue"),
  },
  props: ["reloadTable"],
  data: () => ({
    search: "",
  ...
  mounted() {
    this.eventHub.$on("closeOICDialog", () => {
      this.initialize();
    });
  },
  beforeDestroy() {
    this.eventHub.$off("closeOICDialog");
  },
  methods: {
    pagination(data) {
    ...
    initialize() {
      this.loading = true;
      this.axiosCall("/my-oic", "GET").then((res) => {
        if (res) {
