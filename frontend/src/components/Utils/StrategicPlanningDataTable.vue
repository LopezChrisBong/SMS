<template>
  <div>
    ...
    <AddStrategicPlanningDialog :data="itemData" :action="action" />
    <ViewStrategicPlanning :data="ipcrData" :action="action" />
...
export default {
  components: {
    AddStrategicPlanningDialog: () =>
      import(
        "../../components/Dialogs/Forms/StragtegicPlanning/AddStrategicPlanningDialog.vue"
      ),
    ViewStrategicPlanning: () =>
      import(
        "../../components/Dialogs/Forms/StragtegicPlanning/ViewStrategicPlanning.vue"
      ),
  },
  data: () => ({
    confirmDialog: false,
  ...
  mounted() {
    this.eventHub.$on("closeStrategicPriority", () => {
      this.initialize();
    });

    this.eventHub.$on("closeViewStrategicPlanning", () => {
      this.initialize();
    });
  },

  beforeDestroy() {
    this.eventHub.$off("closeStrategicPriority");
    this.eventHub.$off("closeViewStrategicPlanning");
  },
  watch: {
    options: {
      handler() {
  ...
  methods: {
    ...
    initialize() {
      this.axiosCall("/strategic-planning", "GET").then((res) => {
        if (res) {
          this.data = res.data;
          this.loading = false;
        }
