<template>
  <div>
    ...
    <IldpCalendarDialog :data="updateData" :action="action" />
...
export default {
  components: {
    IldpCalendarDialog: () =>
      import("../../components/Dialogs/Forms/IldpCalendarDialog.vue"),
  },
  props: ["reloadTable"],
  data: () => ({
    search: "",
  ...
  mounted() {
    this.eventHub.$on("closeIldpCalendarDialog", () => {
      this.initialize();
    });
  },
  beforeDestroy() {
    this.eventHub.$off("closeIldpCalendarDialog");
  },
  methods: {
    pagination(data) {
    ...
    initialize() {
      this.loading = true;
      this.axiosCall("/ildp-calendar-year", "GET").then((res) => {
        if (res) {
          this.data = res.data;
          this.loading = false;
        }
