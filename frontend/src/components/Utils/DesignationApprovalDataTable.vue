<template>
  <div>
    ...
    <DesignationApprovalDialog :data="viewData" :action="action" />
...
export default {
  components: {
    DesignationApprovalDialog: () =>
      import("../../components/Dialogs/Forms/DesignationApprovalDialog.vue"),
  ...
  mounted() {
    this.eventHub.$on("closeDesignationApprovalDialog", () => {
      this.initialize(this.tab);
    });
  },
  beforeDestroy() {
    this.eventHub.$off("closeDesignationApprovalDialog");
  },
  ...
  methods: {
    ...
    initialize(status) {
      ...
      this.axiosCall(
        "/my-designation/getDesignationByStatus/" + status,
        "GET"
      ).then((res) => {
        if (res) {
