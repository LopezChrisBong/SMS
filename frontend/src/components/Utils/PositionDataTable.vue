  ...
  <div>
    ...
    <v-card class="ma-5 dt-container" elevation="0" outlined>
      <v-data-table
        ...
        hide-default-footer
      >
        <template v-slot:[`item.is_teaching`]="{ item }">
          <v-chip
            small
            :color="item.status ? '#FF9135' : '#17C800'"
            text-color="white"
          >
            {{ item.is_teaching ? "Teaching" : "Non-Teaching" }}
          </v-chip>
        </template>
    ...
    <PositionDialog :data="updateData" :action="action" />
...
export default {
  ...
  data: () => ({
    search: "",
    headers: [
      { text: "Position", value: "description", align: "start" },
      { text: "Type", value: "is_teaching", align: "center" },
      {
        text: "Actions",
        value: "actions",
        align: "center",
  ...
  mounted() {
    this.eventHub.$on("closePositionDialog", () => {
      this.initialize();
    });
  },
  beforeDestroy() {
    this.eventHub.$off("closePositionDialog");
  },
  methods: {
    pagination(data) {
