<template>
  <div>
    ...
    <TARApprovalDialog :data="viewData" :action="action" :isHRApproval="true" />
...
export default {
  components: {
    TARApprovalDialog: () =>
      import("../../components/Dialogs/Forms/TARApprovalDialog.vue"),
  },
  data: () => ({
    search: "",
    headers: [
  ...
  mounted() {
    this.eventHub.$on("closeTARApprovalDialog", () => {
      this.initialize(this.tab);
    });
  },
  beforeDestroy() {
    this.eventHub.$off("closeTARApprovalDialog");
  },
  ...
  methods: {
    ...
    initialize(status) {
      ...
      this.axiosCall("/tar/getTARToApproveForHR/" + status, "GET").then(
        (res) => {
          if (res) {
            let data = res.data;
