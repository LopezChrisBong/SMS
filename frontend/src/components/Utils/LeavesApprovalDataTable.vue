...
export default {
  components: {
    ViewLeaveApprovalDialog: () =>
      import("../../components/Dialogs/Views/ViewLeaveApprovalDialog.vue"),
  },
  data: () => ({
    search: "",
    headers: [
  ...
  mounted() {
    this.eventHub.$on("closeViewLeaveApprovalDialog", () => {
      this.initialize(this.tab);
    });
  },
  beforeDestroy() {
    this.eventHub.$off("closeViewLeaveApprovalDialog");
  },
  ...
  methods: {
    ...
    initialize(status) {
      ...
      this.axiosCall("/leaves/getLeaveApproval/" + status, "GET").then(
        (res) => {
          if (res) {
