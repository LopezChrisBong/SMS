  ...
  <div>
    ...
    <v-card class="ma-5 dt-container" elevation="0" outlined>
      <v-data-table
        ...
        hide-default-footer
      >
        <template v-slot:[`item.mnthyr`]="{ item }">
          {{ monthList[item.mnth - 1].name + " " + item.year }}
        </template>
    ...
    <ViewMPORApprovalDialog :data="ipcrMpor" :action="action" />
...
export default {
  components: {
    ViewMPORApprovalDialog: () =>
      import("../../components/Dialogs/Views/ViewMPORApprovalDialog.vue"),
  },
  data: () => ({
    search: "",
    headers: [
  ...
  beforeDestroy() {
    this.eventHub.$off("closeViewMPORApprovalDialog");
  },
  ...
  methods: {
    ...
    initialize(status) {
      ...
      this.loading = true;
      this.axiosCall(
        "/ipcr-mpor/getApprovalByStatus/" + filter + "/" + status,
        "GET"
      ).then((res) => {
        if (res) {
          let data = res.data;
      ...
      //         };
      //         this.paginationData1 = paginationData;
