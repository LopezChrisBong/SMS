<template>
  <div>
    ...
    <DesignationDialog :data="updateData" :action="action" />
...
export default {
  ...
  data: () => ({
    ...
    headers: [
      { text: "Designation", value: "description", align: "start" },
      {
        text: "Actions",
        value: "actions",
        align: "center",
  ...
  mounted() {
    this.eventHub.$on("closeDesignationsDialog", () => {
      this.initialize();
    });
  },
  beforeDestroy() {
    this.eventHub.$off("closeDesignationsDialog");
  },
  methods: {
    pagination(data) {
