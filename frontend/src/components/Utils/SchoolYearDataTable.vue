<template>
  <div>
    ...
    <SchoolYearDialog :data="updateData" :action="action" />
...
export default {
  components: {
    SchoolYearDialog: () =>
      import("../../components/Dialogs/Forms/SchoolYearDialog.vue"),
  },
  props: ["reloadTable"],
  data: () => ({
    search: "",
    headers: [
      { text: "FROM", value: "syFrom", align: "start" },
      { text: "TO", value: "syTo", align: "start" },
      {
        text: "Actions",
        value: "actions",
        align: "center",
  ...
  mounted() {
    this.eventHub.$on("closeSchoolYearDialog", () => {
      this.initialize();
    });
  },
  beforeDestroy() {
    this.eventHub.$off("closeSchoolYearDialog");
  },
  methods: {
    add() {
