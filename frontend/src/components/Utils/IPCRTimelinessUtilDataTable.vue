<template>
  <div>
    ...
    <IPCRQualityUtilDialog :data="updateData" :action="action" />
...
export default {
  components: {
    IPCRQualityUtilDialog: () =>
      import("../../components/Dialogs/Forms/IPCRTimelinessUtilDialog.vue"),
  },
  data: () => ({
    search: "",
    headers: [
  ...
  mounted() {
    this.eventHub.$on("closeIPCRTimelinessUtilDialog", () => {
      this.initialize();
    });
  },
  beforeDestroy() {
    this.eventHub.$off("closeIPCRTimelinessUtilDialog");
  },
  methods: {
    pagination(data) {
