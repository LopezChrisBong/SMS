    ...
    getInvoiceSummary(wds) {
      ...
      this.axiosCall("/client-invoice/getInvoiceSummary", "POST", data).then((res) => {
        if (res) {
          ...
          let summary = [];
          console.log("HAHAHAHHA", res.data.workdetails)
          res.data.workdetails.forEach(item => {
            summary.push({
              ...
              qty: JSON.parse(item.workdetail.technicianID).length,
              hours: item.workdetail.actual_work_hour,
              unitPrice: item.clientData.rate,
              amount: parseFloat(item.clientData.rate) * 1 * parseFloat(item.workdetail.actual_work_hour)
            })
            if(item.expenses){
