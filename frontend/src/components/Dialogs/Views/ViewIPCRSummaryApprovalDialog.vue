    ...
    >
      <v-form ref="MPORSummary" @submit.prevent>
        <v-card>
          ...
          <v-divider></v-divider>
          <v-card-actions v-if="approval_type == 1" class="pa-5">
            <v-spacer></v-spacer>
            <v-btn color="red" class="white--text" @click="pending()">
              Pending
            </v-btn>
  ...
  methods: {
    initialize() {
      ...
      if (this.tab != 3) {
        ...
        if (this.ipcr_ID) {
          this.axiosCall(
            "/ipcr-target/getMyIPCRSupportSummary/" + this.ipcr_ID,
            "GET"
          ).then((res) => {
            if (res) {
      ...
      //load signatory
      this.axiosCall(
        "/ipcr/getIPCRApproverNames/" + this.ipcr_ID + "/SUMMARY",
        "GET"
      ).then((res) => {
        if (res) {
    ...
    saveApprovalRemarks() {
      if (this.$refs.RemarksFormref.validate()) {
        ...
        let data = {
          ...
          ipcr_ID: this.ipcr_ID,
        };
        this.axiosCall("/ipcr/approveSummary", "POST", data).then((res) => {
    ...
    savePendingRemarks() {
      if (this.$refs.RemarksFormref.validate()) {
        ...
        this.axiosCall("/ipcr/pendingSummary", "POST", data).then((res) => {
          if (res.data.status == 200) {
            this.confirmBtnLoading = false;
            this.fadeAwayMessage.show = true;
            this.fadeAwayMessage.type = "success";
    ...
    closeD() {
      this.eventHub.$emit("closeIPCRSummaryDialog", false);
      this.confirmSubmitDialog = false;
