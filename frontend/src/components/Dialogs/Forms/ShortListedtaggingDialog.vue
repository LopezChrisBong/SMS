      ...
      <v-form ref="employeeDialogForm" @submit.prevent>
        <v-card>
          ...
          <v-card-actions class="pa-5">
            ...
            <v-btn color="#147452" class="white--text" @click="saveEmployees()">
              <v-icon>mdi-check-circle</v-icon>
              Save
            </v-btn>
          </v-card-actions>
...
export default {
  ...
  methods: {
    ...
    getTaggedEmployee(id) {
      this.axiosCall("/short-list/getByShortList/" + id, "GET").then((res) => {
        if (res.data) {
          let data = res.data;
          ...
          this.employee1 = data;
          console.log("Employeee", this.employee1);
        }
      });
    },
    ...
    closeD() {
      this.eventHub.$emit("closeShortListDialog", false);
      this.initialize();
      this.dialog = false;
    },
    ...
    save() {
      ...
      this.axiosCall("/short-list", "POST", data).then((res) => {
        console.log(res);
