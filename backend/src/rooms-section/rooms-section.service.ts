...
export class RoomsSectionService {
  ...
  async create(createRoomsSectionDto: CreateRoomsSectionDto) {
    ...
    try {
      const count = await this.dataSource.query(
        'SELECT COUNT(*) as count FROM rooms_section where room_section ="' +
          createRoomsSectionDto.room_section +
          '"',
      );
      if (count[0].count == 0) {
      ...
      return {
        msg: 'Duplicate room name, please check if the room already exist!',
  ...
  async findAllAddedRooms(curr_user: any) {
    ...
    const data = await this.dataSource.manager
      ...
      .orderBy(
        `FIELD(RS.grade_level, ${grade.map((g) => `'${g}'`).join(', ')})`,
      )
        ...
        gradeRecord.forEach((student) => {
          ...
          if (student.strand && student.strand !== '0') {
            matchedRoom = rooms.find(
              (r) => String(r.strandId) === String(student.strand),
            );
          }
  ...
  async findAllRoomsSection(id: number) {
    // console.log(id);
